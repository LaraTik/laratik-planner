...
| 8   | Threaded discussion, mentions, attachments, notifications, outbox events          | ✅           | Discussion section on content detail page (post / reply / resolve + visibility labels), @mention extraction, outbox events, in-app notifications bell with badge + popover + mark-read actions, dispatcher loop. **4 production bugs caught + fixed** during this pass: (1) the `user_email_format` CHECK + the `outbox_event.payload` column silently serialised as TEXT instead of jsonb by Drizzle — same root cause, same fix class as the email-check fix (use the proper Drizzle builder, not a custom helper that bypasses type emission), (2) `scripts/dev.sh reset` used `docker compose down -v` which destroys the data volume — replaced with a schema-only reset. |
...
| 12  | Responsive completion, WCAG 2.2 AA, axe-core per route, perf budget               | ✅ (basic)   | axe-core per-route test scaffolded, WCAG 2.2 AA tags; full E2E suite (46 tests) covers public, auth-gate, workspace, content, a11y, discussions + notifications; **3 production bugs caught + fixed** during the e2e pass: (1) `user_email_format` CHECK rejected real emails (`\s` serialised as `s` by Drizzle — switched to `[[:space:]]`), (2) `createWorkspaceAction` silently rolled back writes because `redirect()` was inside a transaction, (3) `--fg-muted` failed WCAG AA contrast (3.51:1 on canvas → fixed to 5.71:1).                                                                                                                                           |
| 13  | Security hardening, Sentry, restic offsite backups, CI hardening, recovery drills | ✅ (partial) | Sentry wrapper wired (env-gated no-op without `SENTRY_DSN`), CI now includes a Playwright e2e job on every push, `.env` secrets never logged. Skipped: restic offsite (needs user repo) + `@sentry/nextjs` source-map upload at build time.                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
- Optional observability: set `SENTRY_DSN` in `/opt/laratik-planner/.env` (the wrapper is already wired + tested)
- Optional backups: set up restic offsite per the runbook (script already in place)
