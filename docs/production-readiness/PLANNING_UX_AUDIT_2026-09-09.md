...
| Assets            | The screenshot has a delivery/version card, but review ownership and next step are easy to miss.                | `DeliverySection` owns stored media selection, version history, submit flow, and approval data.                                                                                                         | Stitch uses version switching, review state, and an approval gate.                             | Improve wording and evidence in follow-up pass; do not change delivery semantics.                    |
| Publish           | The screenshot combines setup, missing requirements, channel packages, and preview.                             | `PublishPackageForm` has per-channel tabs, explicit Save, readiness blockers, approval, and publish outcome recording.                                                                                  | Stitch favors a pre-flight checklist and explicit publish confirmation.                        | Keep current form and make each blocker resolve to a specific field/section in the next pass.        |
| Creation          | Quick Create has more than the four core fields visually because channel defaults are offered.                  | Server action contract keeps title, format, planned date, and brief; channel defaults are optional supporting setup. Batch add validates rows before submit and preserves 4-column paste compatibility. | Stitch treats Quick Create as a fast entry point and Batch Add as a previewable import.        | Preserve the four-field contract; clarify optional defaults and post-create handoff.                 |
