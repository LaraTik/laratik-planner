...
| `pnpm verify`; `actionlint`; `zizmor`; `git diff --check`                    | `04f045b` change snapshot                  | Pass: 133 files / 1,545 tests and production build; release automation now uses explicit repository API paths, creates immutable tags through the GitHub API, refuses tag movement and is idempotent without persisting checkout credentials.                                                              |
