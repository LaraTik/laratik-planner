...
| `AI_FEATURE_ENABLED`       | no       | `false`                            | Env-managed AI path kill-switch. When `false` AND no `MINIMAX_API_KEY` AND no agency has a managed secret, AI endpoints return 503. A managed secret set at `/app/agency-settings/ai` is an explicit agency-admin action and bypasses this kill-switch (the operator's "yes" is the secret itself). Use `AI_FEATURE_ENABLED=false` only when you want to forbid ALL agency AI usage — for selective disable, leave `AI_FEATURE_ENABLED=true` and let the per-agency master switch do the work. |
| `AI_SECRET_ENCRYPTION_KEY` | no       | (auto-file fallback)               | M3.4 — KEK for the per-agency in-DB managed AI secret at `/app/agency-settings/ai`. **Set this in production** so the encryption layer does not try to auto-write `<LARATIK_DATA_DIR>/kek.json` (fails with EACCES inside the container). ≥ 32 ASCII bytes. Generate with `openssl rand -base64 32`.                                                                                                                                                                                           |
| `LARATIK_DATA_DIR`         | no       | `<cwd>/.laratik-planner`           | Directory the auto-managed KEK file lives in when `AI_SECRET_ENCRYPTION_KEY` is unset. MUST be on a persistent volume in production (the file is the master key for every stored AI provider key). Leave empty when the env var is set.                                                                                                                                                                                                                                                        |
