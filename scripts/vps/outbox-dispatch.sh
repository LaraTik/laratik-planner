#!/usr/bin/env bash
# scripts/vps/outbox-dispatch.sh
#
# Notification outbox cron caller. Invokes the authenticated route
# /api/cron/outbox once per cron tick (every minute per
...
: "${CRON_SECRET:?CRON_SECRET is required for the outbox cron caller}"

APP_BASE_URL="${APP_BASE_URL:-http://127.0.0.1:3100}"
ENDPOINT="${APP_BASE_URL%/}/api/cron/outbox"

# -s silent, -S show errors, -X POST so the route accepts either verb,
# -o write body, -w write status, --max-time 60s.
HTTP_RESPONSE="$(curl --silent --show-error --max-time 60 \
  --request POST \
  --header "Authorization: Bearer ${CRON_SECRET}" \
  --write-out '\n__HTTP_STATUS__:%{http_code}\n' \
  "${ENDPOINT}" 2>&1)" || {
  echo "[outbox] curl failed to reach ${ENDPOINT}" >&2
  exit 1
}
...
case "${HTTP_STATUS}" in
  2*)
    ...
    ;;
  401)
    echo "[outbox] 401 from ${ENDPOINT} — check CRON_SECRET and proxy auth" >&2
    exit 2
    ;;
  5*)
    echo "[outbox] ${HTTP_STATUS} from ${ENDPOINT}: ${HTTP_BODY}" >&2
    exit 3
    ;;
  *)
    echo "[outbox] unexpected ${HTTP_STATUS} from ${ENDPOINT}: ${HTTP_BODY}" >&2
    exit 4
    ;;
esac
