...
| §4 (Storage)       | Supabase Storage                        | Database-configured, private Cloudflare R2 via S3-compatible API; local volume retained only for migration rollback | R2 provides private object storage, direct browser uploads, signed reads, and zero-egress delivery without putting provider credentials in deployment environment variables |
