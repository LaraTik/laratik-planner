...
- **Media Library preview signing no longer fans out per asset.** The
  previous Tier 3 page render performed an authorization lookup, preview
  lookup, storage-context resolution, and URL-signing setup for every card.
  The page now batches preview-object resolution, resolves the R2 context
  once per agency, signs concurrently, and reuses presigned URLs for their
  safe lifetime with a bounded process-local cache. Workspace and bucket
  checks remain enforced before a URL is returned; callers still perform
  application-level access checks through `listMediaAssets`. Added a
  regression test covering one batch query/context resolution for a page of
  objects.
...
- **Media Library page emits signed URLs.** The server-side page
  calls the batch `getSignedPreviewUrls` helper for rows with preview
  variants, which attaches `signedPreviewUrl` to each row. `MediaCard` +
  `MediaListRow` prefer that field. A 48-card page now makes **zero**
  requests to the Next.js image path on first paint while avoiding the
  previous per-row database/configuration fan-out.
