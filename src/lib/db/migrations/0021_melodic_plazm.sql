...
--
-- The readiness probe is now lenient about extra migration rows
-- (it checks that every expected timestamp is applied, not that
-- the count matches exactly), so orphan hashes from the previous
-- failed deploy do not trip the health check.
