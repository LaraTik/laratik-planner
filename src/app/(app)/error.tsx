  ...
  return (
    ...
    >
        ...
        <Card padding="md" data-testid="app-error-hint" className="md:col-span-3">
          ...
          {hint.runbookHint ? (
            <p className="text-label text-fg-muted mt-3">
              Search the runbook for{" "}
              <code className="bg-surface rounded px-1.5 py-0.5 font-mono">{hint.runbookHint}</code>
              .
            </p>
          ) : null}
        </Card>
      ...
      <EmptyState
        ...
        description={`Copy the full report (button at the top) and paste it into a support ticket or the runbook search at docs/operations/runbook.md. The reference above is the fastest way for the team to find the matching event.`}
      />
    </div>
  );
}
