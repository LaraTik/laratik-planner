...
import { bulkSetMediaAssetTags, MediaPermissionError } from "@/lib/media/service";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await bulkSetMediaAssetTags(actor, {
      assetIds: parsed.data.assetIds,
      ...(parsed.data.add ? { add: parsed.data.add } : {}),
      ...(parsed.data.remove ? { remove: parsed.data.remove } : {}),
    });
